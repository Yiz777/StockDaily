...
1. **可点击的英文事实句**：不是只贴标题，而是像 2026-08-14 至 2026-08-20 的日报一样，用 1–2 句英文压缩关键事实、数字、预期差和因果；整句链接到报道、监管文件或公司公告，不链接搜索结果页。
2. **来源 + 发布时间**：交易日优先过去 36 小时；周末/休市可放宽到 72 小时，并注明日期。
3. **大白话翻译**：必须保留关键数字和预期差，用自然口语把“发生了什么、为什么发生”讲成完整一段；不能退化成一句干巴巴摘要。
4. **持仓因果链**：用「A → B → 对哪只持仓/现金节奏有什么影响」写清结论；必须落到 VOO / QQQ / SCHD / WDC / MCD / UNH、现金节奏或明确触发线，没有影响就不收录。
...
### 写作风格：必须复刻 2026-08-14 至 2026-08-20

每条严格保持旧版三段式，不增加一整行“来源卡片”：

1. `original`：高密度英文事实句，包含数字、对比或预期差；末尾用小字标来源和时间，整句可点击。
2. `simple`：以 `💬` 开头，用2–3句中文口语解释，不丢关键数字，要让小白知道事情为什么发生。
3. `impact`：以 `→` 开头，用明确因果链连接到具体持仓和动作，语气可以鲜明，但不能夸大或把相关性写成因果。

一组新闻优先覆盖不同层次：①宏观政策/经济数据，②债券、商品或板块反应，③持仓公司真实事件，④必要时补消费/AI/医疗等交叉验证。不要连续三条都在复述同一个FOMC事件。

目标是 4–5 条高质量新闻。若当天只能核验 2–3 条，就明确写“今天仅核验到 N 条有效新闻”，**宁缺毋滥，不得用无效内容补足数量**。
...
| 7 | 新闻翻译官 | 📰 | **目标4–5条有效新闻；复刻8月中旬的英文事实句→大白话→持仓因果链，保留来源时间直链** |
| 8 | 持仓速览 | 👜 | 蓝色背景，无账号股数 |
